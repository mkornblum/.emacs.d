...
** Flycheck
On the fly syntax checking for languages where we don't use LSP servers (cough Ruby)
#+begin_src emacs-lisp
...
#+end_src
** Editor Config
play nicely with project config even when other people use a different editor
#+begin_src emacs-lisp
...
# #+end_src
*** Tree-sitter
Using Emacs 29+ built-in treesit via treesit-auto.
#+begin_src emacs-lisp
...
#+end_src
*** Format all mode
Auto-format all the code with one command, regardless of language https://github.com/lassik/emacs-format-all-the-code
#+begin_src emacs-lisp
...
Treesitter TS fix via https://genehack.blog/2024/02/fixing-an-emacs-typescript-ts-mode-problem/
#+begin_src emacs-lisp
...
** Meow
Let's try modal editing again, but be even more esoteric than evil/vim! See keybindings
above provided by "meow-qwerty".
#+begin_src emacs-lisp
(use-package meow
  :config
...
#+end_src
** Ace Window
Nice for moving from window to window quickly
#+begin_src emacs-lisp
...
** Embark 
I found I missed this after a while, so adding it back again 
#+begin_src emacs-lisp
(use-package embark
  :ensure t
...
installed this mostly to make org-mode HTML exports nicer, but maybe it will be useful for other things too?
#+begin_src emacs-lisp
(use-package htmlize)
#+end_src
